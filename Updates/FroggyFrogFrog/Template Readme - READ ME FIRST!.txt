...
Ver 1.95 - Rephrased installing an rss feed in etc.dic.
Ver 1.94 - Updated yaya.dll to Tc571-5 and removed/added required files, used the : all tag to clean up some menu set-up in menu.dic. Took out name/pronouns from first boot, made pronouns/birthday opt-in by default, made extra checks in OnBoot and OnClose optional, trimmed down if checks in aitalk, made time checks optional, made screen boundary check opt-in, made titles optional, rewrote parts of etc.dic.
Ver 1.93 - Fixed the description for always in surfaces.txt.
Ver 1.92 - Added a thing in OnUpdateFailure that will tell you which file isn't matching when updating.
Ver 1.91 - Moved OnChoiceSelect out of the shiori file and updated it, updated yaya.dll, renamed AiTalk to OnAiTalk, added nonoverlap_pool to the RandomTalk section, removed the notes about overlapping filenames in surfaces.txt, removed a green line from one image file.
Ver 1.90 - Added OnWindowStateMinimize to bootend.dic.
Ver 1.89 - Fixed the if check for Sunday dialogue in aitalk.dic.
Ver 1.88 - Added responses for setting up headlines or rss feeds in etc.dic.
Ver 1.87 - Removed getnowmin and getnowhour from menu.dic.
Ver 1.86 - Added directions for how to set up groups and unique names in surfacetable.txt in the shell folder.
